import requests

""" 
Este archivo tiene caracter de prueba, 
para una visualizacion mas sencilla de como utilizar la libreria request.
"""

url = ' https://apis.datos.gob.ar/georef/api/provincias' 
res= requests.get(url) # => Objeto donde vamos a almacenar la respuesta
data = res.json()
print("\n||||||||||||||||||||||||| RESPUESTA API TRANSFORMADO A JSON ||||||||||||||||||||||||||||||||||||||||||||||||||")  
print(data)

print("\n||||||||||||||||||||||||| CANTIDAD DE PROVINCIAS ||||||||||||||||||||||||||||||||||||||||||||||||||")  
print(data["cantidad"]) 

pcias = data["provincias"]
nombres_pcias = []
for i in pcias:    
    nombre = i["nombre"]
    nombres_pcias.append(nombre)
    
print("\n||||||||||||||||||||||||| NOMBRES PROVINCIAS ||||||||||||||||||||||||||||||||||||||||||||||||||")    
print(nombres_pcias) 
print("\n")

lat_alta = []
lat_baja = []
for i in pcias:
    if i["centroide"]["lat"] > -41.1649750616583:
        lat_alta.append({
            "Provincia": str(i["nombre"]), 
            "Latitud": str(i["centroide"]["lat"])
            })
    else:
        lat_baja.append({
            "provincia": str(i["nombre"]), 
            "latitud": str(i["centroide"]["lat"])
            })

print("\n||||||||||||||||||||||||| LATITUDES ALTAS ||||||||||||||||||||||||||||||||||||||||||||||||||")
print(lat_alta)
print("\n")
print("\n|||||||||||||||||||||||||LATITUDES BAJAS||||||||||||||||||||||||||||||||||||||||||||||||||")
print(lat_baja)



{
   "cantidad":24,
   "inicio":0,
   "parametros":{
      
   },
   "provincias":[
      {
         "centroide":{
            "lat":-34.6144420654301,
            "lon":-58.4458763250916
         },
         "id":"02",
         "nombre":"Ciudad Autónoma de Buenos Aires"
      },
      {
         "centroide":{
            "lat":-38.6419828626673,
            "lon":-70.1198972237318
         },
         "id":"58",
         "nombre":"Neuquén"
      },
      {
         "centroide":{
            "lat":-33.7611035381154,
            "lon":-66.0252312714021
         },
         "id":"74",
         "nombre":"San Luis"
      },
      {
         "centroide":{
            "lat":-30.7088227091528,
            "lon":-60.9506872769706
         },
         "id":"82",
         "nombre":"Santa Fe"
      },
      {
         "centroide":{
            "lat":-29.6849372775783,
            "lon":-67.1817575814487
         },
         "id":"46",
         "nombre":"La Rioja"
      },
      {
         "centroide":{
            "lat":-27.3359537960762,
            "lon":-66.9478972451295
         },
         "id":"10",
         "nombre":"Catamarca"
      },
      {
         "centroide":{
            "lat":-26.948283501723,
            "lon":-65.3647655803683
         },
         "id":"90",
         "nombre":"Tucumán"
      },
      {
         "centroide":{
            "lat":-26.3869871835867,
            "lon":-60.765116260356
         },
         "id":"22",
         "nombre":"Chaco"
      },
      {
         "centroide":{
            "lat":-24.8950871761481,
            "lon":-59.9321901121647
         },
         "id":"34",
         "nombre":"Formosa"
      },
      {
         "centroide":{
            "lat":-48.8155471830527,
            "lon":-69.9557619144913
         },
         "id":"78",
         "nombre":"Santa Cruz"
      },
      {
         "centroide":{
            "lat":-43.7886271389083,
            "lon":-68.5267363339818
         },
         "id":"26",
         "nombre":"Chubut"
      },
      {
         "centroide":{
            "lat":-34.6303887067166,
            "lon":-68.5829456019867
         },
         "id":"50",
         "nombre":"Mendoza"
      },
      {
         "centroide":{
            "lat":-32.0589278938558,
            "lon":-59.201262616496
         },
         "id":"30",
         "nombre":"Entre Ríos"
      },
      {
         "centroide":{
            "lat":-30.8656607015096,
            "lon":-68.8881597071776
         },
         "id":"70",
         "nombre":"San Juan"
      },
      {
         "centroide":{
            "lat":-23.3199750616583,
            "lon":-65.764423919292
         },
         "id":"38",
         "nombre":"Jujuy"
      },
      {
         "centroide":{
            "lat":-27.7834318817521,
            "lon":-63.2526268856462
         },
         "id":"86",
         "nombre":"Santiago del Estero"
      },
      {
         "centroide":{
            "lat":-40.4050796306359,
            "lon":-67.2296757996036
         },
         "id":"62",
         "nombre":"Río Negro"
      },
      {
         "centroide":{
            "lat":-28.7742044813623,
            "lon":-57.8010818603331
         },
         "id":"18",
         "nombre":"Corrientes"
      },
      {
         "centroide":{
            "lat":-26.8753025989034,
            "lon":-54.6515705627219
         },
         "id":"54",
         "nombre":"Misiones"
      },
      {
         "centroide":{
            "lat":-24.2992838957201,
            "lon":-64.8141586574346
         },
         "id":"66",
         "nombre":"Salta"
      },
      {
         "centroide":{
            "lat":-32.1447993873859,
            "lon":-63.801973466573
         },
         "id":"14",
         "nombre":"Córdoba"
      },
      {
         "centroide":{
            "lat":-36.6773920760823,
            "lon":-60.5584771084959
         },
         "id":"06",
         "nombre":"Buenos Aires"
      },
      {
         "centroide":{
            "lat":-37.1350652212898,
            "lon":-65.4476439990213
         },
         "id":"42",
         "nombre":"La Pampa"
      },
      {
         "centroide":{
            "lat":-82.5211345211545,
            "lon":-50.7428606764691
         },
         "id":"94",
         "nombre":"Tierra del Fuego, Antártida e Islas del Atlántico Sur"
      }
   ],
   "total":24
}